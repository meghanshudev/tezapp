...
class _MemberRequestPageState extends State<MemberRequestPage> {
  List groupMember = [];
  var groupDatMember = '';
  bool isLoadingAcceptButton = false;
  bool isLoadingRejectButton = false;
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              !checkIsNullValue(widget.data['member']['name'])
                  ? widget.data['member']['name']
                  : "N/A",
          ...
          Padding(
              padding: const EdgeInsets.only(left: 15, right: 15),
              child: Text(
                !checkIsNullValue(widget.data['member']['address'])
                    ? widget.data['member']['address']
                    : "N/A",
          ...
          Padding(
              padding: const EdgeInsets.only(left: 15, right: 15),
              child: Text(
                !checkIsNullValue(widget.data['member']['zip_code'])
                    ? widget.data['member']['zip_code']
                    : "N/A",
                style: smallBlackText,
              )),
          SizedBox(
          ...
          Padding(
              padding: const EdgeInsets.only(left: 15, right: 15),
              child: Text(
                !checkIsNullValue(widget.data['member']['phone_number'])
                    ? widget.data['member']['phone_number']
                    : "N/A",
                style: smallBlackText,
              )),
          Expanded(
  ...
  accept() async {
    if (isLoadingAcceptButton) return;
    setState(() {
      isLoadingAcceptButton = true;
    });
    var groupId = userSession['group']['id'];
    var requestId = widget.data['id'].toString();
    var response = await netPost(
      isUserToken: true,
      endPoint: "group/$groupId/accept/$requestId",
      params: {},
    );
    ...
    if (mounted) {
      if (response['resp_code'] == "200") {
        notifyAlert(context,
            desc: "you_have_accepted_a_new_member".tr(),
  ...
  decline() async {
    if (isLoadingRejectButton) return;
    setState(() {
      isLoadingRejectButton = true;
    });
    var groupId = userSession['group']['id'];
    var requestId = widget.data['id'].toString();
    var response = await netPost(
      isUserToken: true,
      endPoint: "group/$groupId/reject/$requestId",
      params: {},
    );
    ...
    if (mounted) {
      if (response['resp_code'] == "200") {
        notifyAlert(context,
            desc: "you_have_rejected_a_new_member".tr(),
            btnTitle: "Ok", onConfirm: () {
          Navigator.pop(context);
          Navigator.pop(context);
        });
